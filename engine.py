...
def run_backtest(etf_data, universo_long, universo_short, n_max,
    ...
    for idx, rdate in enumerate(rebal_dates):
        ...
        for etf in universo_long:
            ...
            cand_long.append({"ticker":t,"nome":etf.get("nome",t),"cat":etf.get("cat",""),
        ...
        for c in top_long:
            c["peso"] = round(c["score"]**PESO_EXP / tot_wl * peso_disp * ql, 2)
        for c in mem_long:
        ...
        if idx > 0 and comp_att and rebal_dates[idx-1] < rdate:
            ...
            for pos in comp_att:
                ...
                if pp and pn and pp > 0:
                    ret = (pn-pp)/pp
                    if pos.get("is_short"): ret = -ret
                    ptf_ret += ret * pos["peso"]/100
            capitale = round(capitale*(1+ptf_ret), 2)
